...
// Build the libretro-thumbnails fallback chain for a Named_Boxarts URL.
// Many of our generated thumbnailUrls drop the No-Intro region suffix that the
// libretro-thumbnails repo uses, which is why so many tiles 404 to a blank
// letter. We walk a handful of common region suffixes before giving up.
...
function buildCoverFallbacks(url: string): string[] {
  // Only rewrite libretro-thumbnails Named_Boxarts URLs; leave anything else alone.
  const marker = '/Named_Boxarts/';
  const idx = url.indexOf(marker);
  if (idx === -1) return [url];
  const prefix = url.slice(0, idx + marker.length);
  const fileEncoded = url.slice(idx + marker.length);
  // Strip extension (.png) — keep query string if any.
  const qIdx = fileEncoded.indexOf('?');
  const query = qIdx === -1 ? '' : fileEncoded.slice(qIdx);
  const fileNoQuery = qIdx === -1 ? fileEncoded : fileEncoded.slice(0, qIdx);
  const dot = fileNoQuery.lastIndexOf('.');
  if (dot === -1) return [url];
  const baseEncoded = fileNoQuery.slice(0, dot);
  const ext = fileNoQuery.slice(dot);
  // If the base already contains a region suffix, just use the original.
  const baseDecoded = decodeURIComponent(baseEncoded);
...
function GameCover({ src, alt }: { src: string; alt: string }) {
  const candidates = useMemo(() => buildCoverFallbacks(src), [src]);
