...
function EraCarousel({
  ...
  style = {{ borderColor: `${era.accentBorder}` }
}
    >
  {/* Background image */ }
  < div className = "absolute inset-0 z-0" >
...
<div className="relative z-10 p-6 sm:p-8 min-h-[220px] flex flex-col justify-between" >
  ...
  <div className="flex items-center gap-3 mb-2 flex-wrap" >
    <Icon className="w-6 h-6" style = {{ color: era.accent }} />
      < h3
className = "text-2xl sm:text-3xl font-heading tracking-wide"
style = {{ color: era.accent, WebkitTextFillColor: "unset" }}
            >
  { era.name }
  </h3>
  < Badge
...
style = {{
  borderColor: era.accentBorder,
    color: era.accent,
      background: `${era.accent}15`,
              }}
            >
  ...
  < span className = "text-xs text-[hsl(45,15%,70%)] font-body ml-1" > { era.tagline } </span>
    ...
    < p className = "text-sm text-[hsl(45,15%,80%)] font-body max-w-2xl leading-relaxed" >
      { era.description }
      </p>
      </div>
      ...
      < div className = "flex items-center justify-between mt-5 gap-3 flex-wrap" >
        <Button
            size="sm"
onClick = { visitSection }
className = "gilded-button"
style = {{ background: era.accent, color: "hsl(225,30%,8%)" }}
          >
