...
// ── Era carousel (shared slot for Warlords / Nexus / Armada) ──
function EraDot({ active, onClick, accent, label }: { active: boolean; onClick: () => void; accent: string; label: string }) {
  const base = "h-2 rounded-full transition-all";
  const cls = active ? `${base} w-6` : `${base} w-2 bg-[hsl(45,15%,40%)]`;
  const style = active ? { background: accent } : undefined;
  return <button onClick={ onClick } aria - label={ label } className = { cls } style = { style } />;
}

function EraNavBtn({ onClick, label, children }: { onClick: () => void; label: string; children: React.ReactNode }) {
  const cls = "w-8 h-8 rounded-full border border-[hsl(43,60%,30%)]/40 flex items-center justify-center text-[hsl(45,15%,70%)] hover:text-white hover:border-[hsl(43,85%,55%)] transition-all";
  return <button onClick={ onClick } aria - label={ label } className = { cls } > { children } </button>;
}

function EraCarousel({ activeEra, setActiveEra }: { activeEra: EraKey; setActiveEra: (k: EraKey) => void }) {
  const idx = Math.max(0, ERAS.findIndex((e) => e.key === activeEra));
  const era = ERAS[idx];
  const Icon = era.icon;
  const prev = () => setActiveEra(ERAS[(idx - 1 + ERAS.length) % ERAS.length].key);
  const next = () => setActiveEra(ERAS[(idx + 1) % ERAS.length].key);
  const visitSection = () => { document.getElementById(era.key)?.scrollIntoView({ behavior: "smooth" }); };

  const wrapperStyle = { borderColor: era.accentBorder };
  const headingStyle = { color: era.accent, WebkitTextFillColor: "unset" as const };
  const badgeStyle = { borderColor: era.accentBorder, color: era.accent, background: `${era.accent}15` };
  const ctaStyle = { background: era.accent, color: "hsl(225,30%,8%)" };

  return (
    <div className= "relative rounded-xl overflow-hidden border-2 transition-all duration-500" style = { wrapperStyle } >
      <div className="absolute inset-0 z-0" >
  ...
  < h3 className = "text-2xl sm:text-3xl font-heading tracking-wide" style = { headingStyle } > { era.name } </h3>
    < Badge className = "border text-[10px] uppercase tracking-wide" style = { badgeStyle } > { era.key === "armada" ? "Coming Soon" : "Active" } </Badge>
      < span className = "text-xs text-[hsl(45,15%,70%)] font-body ml-1" > { era.tagline } </span>
        ...
        < p className = "text-sm text-[hsl(45,15%,80%)] font-body max-w-2xl leading-relaxed" > { era.description } </p>
          ...
          < div className = "flex items-center justify-between mt-5 gap-3 flex-wrap" >
            <Button size="sm" onClick = { visitSection } className = "gilded-button" style = { ctaStyle } >
  ...
  < div className = "flex items-center gap-3" >
    ...
    {
      ERAS.map((e) => (
        <EraDot key= { e.key } active = { e.key === era.key } onClick = {() => setActiveEra(e.key)} accent = { era.accent } label = {`Show ${e.name}`} />
              ))}
</div>
  < EraNavBtn onClick = { prev } label = "Previous era" > <ChevronLeft className="w-4 h-4" /> </EraNavBtn>
    < EraNavBtn onClick = { next } label = "Next era" > <ChevronRight className="w-4 h-4" /> </EraNavBtn>
      </div>
      </div>
      </div>
      </div>
