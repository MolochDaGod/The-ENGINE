...
function EraCarousel({ activeEra, setActiveEra }: { activeEra: EraKey; setActiveEra: (k: EraKey) => void }) {
                  ...
                  < span className = "text-xs text-[hsl(45,15%,70%)] font-body ml-1" > { era.tagline } </span>
                    ...
                    < p className = "text-sm text-[hsl(45,15%,80%)] font-body max-w-2xl leading-relaxed" > { era.description } </p>
                      ...
                      < div className = "flex items-center justify-between mt-5 gap-3 flex-wrap" >
                        <Button size="sm" onClick = { visitSection } className = "gilded-button" style = {{ background: era.accent, color: "hsl(225,30%,8%)" }}>
  ...
  < div className = "flex items-center gap-3" >
    ...
    {
      ERAS.map((e, i) => (
        <button key= { e.key } onClick = {() => setActiveEra(e.key)} className = {`h-2 rounded-full transition-all ${i === idx ? 'w-6' : 'w-2 bg-[hsl(45,15%,40%)]'}`} style = { i === idx ? { background: era.accent } : undefined} />
              ))}
</div>
