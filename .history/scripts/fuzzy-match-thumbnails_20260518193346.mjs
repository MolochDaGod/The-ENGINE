...
const GH_HEADERS = { 'User-Agent': 'thumb-fuzzy-match', Accept: 'application/vnd.github+json' };
...
async function fetchTree(repo) {
  ...
  for (const ref of ['master', 'main']) {
    const r = await fetch(`https://api.github.com/repos/libretro-thumbnails/${repo}/git/trees/${ref}?recursive=1`, { headers: GH_HEADERS });
    if (!r.ok) continue;
    const j = await r.json();
    if (!Array.isArray(j.tree)) continue;
    const files = j.tree.filter(t => t.type === 'blob' && t.path.startsWith('Named_Boxarts/') && t.path.endsWith('.png')).map(t => t.path.slice('Named_Boxarts/'.length));
  ...
  for (const ref of ['master', 'main']) {
    const r = await fetch(`https://api.github.com/repos/libretro-thumbnails/${repo}/git/trees/${ref}`, { headers: GH_HEADERS });
    if (!r.ok) continue;
    const j = await r.json();
    if (!Array.isArray(j.tree)) continue;
    const dir = j.tree.find(t => t.type === 'tree' && t.path === 'Named_Boxarts');
    if (!dir) continue;
    const r2 = await fetch(`https://api.github.com/repos/libretro-thumbnails/${repo}/git/trees/${dir.sha}`, { headers: GH_HEADERS });
    if (!r2.ok) continue;
    const j2 = await r2.json();
    if (!Array.isArray(j2.tree)) continue;
    const files = j2.tree.filter(t => t.type === 'blob' && t.path.endsWith('.png')).map(t => t.path);
...
function buildIndex(files) {
  ...
  for (const f of files) {
    ...
    map.get(key).push({ full: f, parens, base });
  }
  return map;
}
