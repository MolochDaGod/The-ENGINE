...
function normKey(s) {
  return (
    s
      ...
      .replace(/[\u2018\u2019\u02bc\u02bb]/g, "'")
      .replace(/[\u201c\u201d]/g, '"')
      // libretro-thumbnails substitutes "_" for "&" in filenames (FAT-safe). Treat as the same token.
      .replace(/\s*[&_]\s*/g, " and ")
      .replace(/[^a-z0-9]+/g, " ")
      .trim()
  );
...
function buildIndex(files) {
  ...
  for (const f of files) {
    ...
    // Article-inverted alias: lets catalog "The Karate Kid" match libretro "Karate Kid, The".
    const am = base.match(/^(.+),\s*(The|A|An)$/);
    if (am) {
      const inverted = `${am[2]} ${am[1]}`;
...
async function main() {
  ...
  for (const { game, repo, current } of targets) {
    ...
    // can still resolve them once the ellipsis is gone).
    ...
    if (!hit)
      for (const v of variants) {
        const e = findPrefixMatch(idx.sortedKeys, idx.exact, normKey(v));
        if (e && e.length) {
          hit = pickBest(e);
          layer = "prefix";
          break;
  ...
  console.log(
    `\nFixed: ${fixed}  (exact=${fixedByLayer.exact}, compact=${fixedByLayer.compact}, prefix=${fixedByLayer.prefix})`,
  );
