...
async function fetchWithRetry(url, attempts = 4) {
  // Anonymous GitHub API allows 60 req/hr; failures from secondary rate-limit return
  // 403 with a "X-RateLimit-Remaining: 0" header. Back off exponentially.
...
function findReversePrefixMatch(sortedKeys, exact, fullTitleKey) {
  // The catalog title CONTAINS an index key as its prefix. Used for Neo Geo entries
  // that concatenate "English Name Japanese Name" - libretro stores only the English
  // half. We pick the LONGEST index key that is a prefix of `fullTitleKey + " "`,
  // and require it to be >= 3 words to avoid spurious "the"/"super" matches.
  const haystack = fullTitleKey + " ";
  let best = null;
  // Binary search to the first key > haystack, then walk backward.
  let lo = 0,
    hi = sortedKeys.length;
  while (lo < hi) {
    const mid = (lo + hi) >>> 1;
    if (sortedKeys[mid] <= haystack) lo = mid + 1;
    else hi = mid;
  }
  for (let i = lo - 1; i >= 0 && i >= lo - 50; i--) {
    const k = sortedKeys[i];
    if (k.length >= haystack.length) continue;
    if (!haystack.startsWith(k + " ")) continue;
    if (k.split(" ").length < 3) continue;
    if (!best || k.length > best.length) best = k;
    break; // sortedKeys is asc, so first match walking back is the longest valid one
  }
  return best ? exact.get(best) || null : null;
}
...
async function main() {
  ...
  // Always-available cross-repo indexes for catalog entries that turn out to be on
  // the wrong platform (Game Gear / Master System / GBC titles miscategorized as
  // Genesis or N64). Preloading these lets cross-repo last-resort find them.
  const ALWAYS_LOAD = [
    "Sega_-_Game_Gear",
    "Sega_-_Master_System_-_Mark_III",
    "Nintendo_-_Game_Boy_Color",
  ];
  for (const r of ALWAYS_LOAD)
    if (!primaryRepos.has(r) && !fallbackRepos.has(r)) fallbackRepos.add(r);
  ...
  function matchInIndex(idx, variants, wasTruncated) {
    ...
    // Layer 4: reverse-prefix ("Real Bout Fatal Fury Real Bout Garou Densetsu" -> "Real Bout Fatal Fury")
    for (const v of variants) {
      const e = findReversePrefixMatch(idx.sortedKeys, idx.exact, normKey(v));
      if (e && e.length) return { hit: pickBest(e), layer: "rprefix" };
    }
    return null;
  }
  ...
  let fixed = 0,
    missed = 0,
    fixedByLayer = { exact: 0, compact: 0, prefix: 0, rprefix: 0 },
    fixedByFallback = 0,
    fixedByCrossRepo = 0;
  const misses = [];
  for (const { game, repo, current } of targets) {
    ...
    if (!result) {
      ...
      for (const r of order) {
        ...
        if (m && m.layer !== "prefix" && m.layer !== "rprefix") {
          // Only allow exact/compact across repos: prefix matches grab wrong subtitles.
          result = m;
          matchedRepo = r;
          wasCrossRepo = true;
          break;
  ...
  console.log(
    `\nFixed: ${fixed}  (exact=${fixedByLayer.exact}, compact=${fixedByLayer.compact}, prefix=${fixedByLayer.prefix}, rprefix=${fixedByLayer.rprefix}; via-fallback=${fixedByFallback}, via-cross-repo=${fixedByCrossRepo})`,
  );
