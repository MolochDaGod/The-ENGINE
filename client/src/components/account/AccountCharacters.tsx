import { useMemo, useState } from "react";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { CHARACTER_PREFABS, type CharacterPrefab, type RaceId, type ClassId } from "@shared/character-prefabs";
import type { PlayerProfile } from "@/lib/player-auth";
import {
  claimCharacter,
  fetchUniverse,
  launchWithCharacter,
  patchCharacter,
  type UniverseCharacter,
  type UniverseSnapshot,
} from "@/lib/universe-api";
import { Loader2, Play, Star, UserPlus } from "lucide-react";

const RACES: { id: RaceId; label: string; icon: string }[] = [
  { id: "human", label: "Human", icon: "👑" },
  { id: "barbarian", label: "Barbarian", icon: "🪓" },
  { id: "elf", label: "Elf", icon: "🧝" },
  { id: "dwarf", label: "Dwarf", icon: "⛏️" },
  { id: "orc", label: "Orc", icon: "👹" },
  { id: "undead", label: "Undead", icon: "💀" },
];

const CLASSES: { id: ClassId; label: string; icon: string }[] = [
  { id: "warrior", label: "Warrior", icon: "⚔️" },
  { id: "mage", label: "Mage", icon: "🧙" },
  { id: "ranger", label: "Ranger", icon: "🏹" },
  { id: "worge", label: "Worge", icon: "🐺" },
];

const factionColors: Record<string, string> = {
  crusade: "border-blue-500/40 bg-blue-950/20",
  fabled: "border-green-500/40 bg-green-950/20",
  legion: "border-red-500/40 bg-red-950/20",
};

export default function AccountCharacters({ player }: { player: PlayerProfile }) {
  const qc = useQueryClient();
  const [raceFilter, setRaceFilter] = useState<RaceId | "all">("all");
  const [classFilter, setClassFilter] = useState<ClassId | "all">("all");
  const [selected, setSelected] = useState<CharacterPrefab | null>(null);
  const [busy, setBusy] = useState<string | null>(null);

  const universeQ = useQuery<UniverseSnapshot>({
    queryKey: ["/api/me/universe"],
    queryFn: fetchUniverse,
  });

  const claimed = universeQ.data?.characters ?? [];
  const claimedByPrefab = useMemo(() => {
    const map = new Map<string, UniverseCharacter>();
    for (const c of claimed) map.set(c.prefabId, c);
    return map;
  }, [claimed]);
  const active = claimed.find((c) => c.isActive) ?? claimed[0] ?? null;

  const filtered = CHARACTER_PREFABS.filter(
    (p) =>
      (raceFilter === "all" || p.race === raceFilter) &&
      (classFilter === "all" || p.classId === classFilter),
  );

  async function claim(prefab: CharacterPrefab) {
    setBusy(`claim-${prefab.id}`);
    try {
      await claimCharacter({
        prefabId: prefab.id,
        displayName: prefab.name,
        setActive: claimed.length === 0,
        loadout: {
          heroId: prefab.id,
          primaryWeapon: "pistol",
          secondaryWeapon: "knife",
        },
      });
      await qc.invalidateQueries({ queryKey: ["/api/me/universe"] });
    } catch (e: any) {
      alert(e?.message || "Failed to claim character");
    } finally {
      setBusy(null);
    }
  }

  async function setActive(char: UniverseCharacter) {
    setBusy(`active-${char.id}`);
    try {
      await patchCharacter(char.id, { isActive: true });
      await qc.invalidateQueries({ queryKey: ["/api/me/universe"] });
    } finally {
      setBusy(null);
    }
  }

  async function play(char: UniverseCharacter, target: "warlords" | "warlordGenesis") {
    setBusy(`play-${target}-${char.id}`);
    try {
      if (!char.isActive) await patchCharacter(char.id, { isActive: true });
      await launchWithCharacter(char, target);
      await qc.invalidateQueries({ queryKey: ["/api/me/universe"] });
    } finally {
      setBusy(null);
    }
  }

  return (
    <div className="space-y-6">
      {/* Active roster summary */}
      <section className="fantasy-panel p-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h3 className="font-heading text-base text-[hsl(45,30%,92%)]" style={{ WebkitTextFillColor: "unset" }}>
              Warlords Era Characters
            </h3>
            <p className="text-xs text-[hsl(45,15%,55%)] font-body mt-1">
              Claim prefabs to your Grudge ID. Active hero launches Warlords / Genesis with loadout params.
            </p>
          </div>
          {active ? (
            <div className="flex flex-wrap gap-2 items-center">
              <Badge className="bg-[hsl(43,85%,55%)]/20 text-[hsl(43,85%,55%)] border-[hsl(43,85%,55%)]/40">
                <Star className="w-3 h-3 mr-1" /> {active.displayName} · Lv{active.level}
              </Badge>
              <Button
                size="sm"
                className="gilded-button text-xs"
                disabled={!!busy}
                onClick={() => play(active, "warlords")}
              >
                {busy?.startsWith("play-warlords") ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Play className="w-3.5 h-3.5 mr-1" />}
                Warlords
              </Button>
              <Button
                size="sm"
                variant="outline"
                className="text-xs border-[hsl(43,60%,30%)]/40"
                disabled={!!busy}
                onClick={() => play(active, "warlordGenesis")}
              >
                Genesis
              </Button>
            </div>
          ) : (
            <span className="text-xs text-[hsl(45,15%,55%)] font-body">No claimed heroes yet — pick one below.</span>
          )}
        </div>
        {claimed.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-2">
            {claimed.map((c) => (
              <button
                key={c.id}
                type="button"
                onClick={() => setActive(c)}
                className={`px-2.5 py-1 rounded text-xs border transition-all ${
                  c.isActive
                    ? "border-[hsl(43,85%,55%)]/50 bg-[hsl(43,85%,55%)]/15 text-[hsl(43,85%,55%)]"
                    : "border-[hsl(43,60%,30%)]/25 text-[hsl(45,15%,60%)] hover:border-[hsl(43,60%,30%)]/50"
                }`}
              >
                {c.displayName}
                {busy === `active-${c.id}` && <Loader2 className="inline w-3 h-3 ml-1 animate-spin" />}
              </button>
            ))}
          </div>
        )}
      </section>

      {/* Filters */}
      <section className="fantasy-panel p-4">
        <div className="flex flex-wrap gap-4">
          <div>
            <div className="text-[10px] text-[hsl(45,15%,55%)] uppercase tracking-wider font-body mb-1.5">Race</div>
            <div className="flex flex-wrap gap-1">
              <button
                onClick={() => setRaceFilter("all")}
                className={`px-2.5 py-1 rounded text-xs font-heading transition-all ${raceFilter === "all" ? "bg-[hsl(43,85%,55%)]/20 text-[hsl(43,85%,55%)] border border-[hsl(43,85%,55%)]/40" : "text-[hsl(45,15%,55%)] border border-[hsl(43,60%,30%)]/20 hover:border-[hsl(43,60%,30%)]/40"}`}
              >
                All
              </button>
              {RACES.map((r) => (
                <button
                  key={r.id}
                  onClick={() => setRaceFilter(r.id)}
                  className={`px-2.5 py-1 rounded text-xs font-heading transition-all ${raceFilter === r.id ? "bg-[hsl(43,85%,55%)]/20 text-[hsl(43,85%,55%)] border border-[hsl(43,85%,55%)]/40" : "text-[hsl(45,15%,55%)] border border-[hsl(43,60%,30%)]/20 hover:border-[hsl(43,60%,30%)]/40"}`}
                >
                  {r.icon} {r.label}
                </button>
              ))}
            </div>
          </div>
          <div>
            <div className="text-[10px] text-[hsl(45,15%,55%)] uppercase tracking-wider font-body mb-1.5">Class</div>
            <div className="flex flex-wrap gap-1">
              <button
                onClick={() => setClassFilter("all")}
                className={`px-2.5 py-1 rounded text-xs font-heading transition-all ${classFilter === "all" ? "bg-[hsl(43,85%,55%)]/20 text-[hsl(43,85%,55%)] border border-[hsl(43,85%,55%)]/40" : "text-[hsl(45,15%,55%)] border border-[hsl(43,60%,30%)]/20 hover:border-[hsl(43,60%,30%)]/40"}`}
              >
                All
              </button>
              {CLASSES.map((c) => (
                <button
                  key={c.id}
                  onClick={() => setClassFilter(c.id)}
                  className={`px-2.5 py-1 rounded text-xs font-heading transition-all ${classFilter === c.id ? "bg-[hsl(43,85%,55%)]/20 text-[hsl(43,85%,55%)] border border-[hsl(43,85%,55%)]/40" : "text-[hsl(45,15%,55%)] border border-[hsl(43,60%,30%)]/20 hover:border-[hsl(43,60%,30%)]/40"}`}
                >
                  {c.icon} {c.label}
                </button>
              ))}
            </div>
          </div>
        </div>
      </section>

      <div className="flex flex-col lg:flex-row gap-6">
        <div className="flex-1">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {filtered.map((p) => {
              const owned = claimedByPrefab.get(p.id);
              return (
                <button
                  key={p.id}
                  type="button"
                  onClick={() => setSelected(p)}
                  className={`fantasy-panel p-3 text-left transition-all hover:rune-glow ${selected?.id === p.id ? "ring-2 ring-[hsl(43,85%,55%)]" : ""} ${factionColors[p.faction]}`}
                >
                  <div className="flex items-center gap-2 mb-2">
                    <div
                      className="w-8 h-8 rounded-lg flex items-center justify-center text-lg"
                      style={{ background: p.classColor + "30" }}
                    >
                      {CLASSES.find((c) => c.id === p.classId)?.icon}
                    </div>
                    <div className="min-w-0">
                      <div className="text-xs font-heading text-[hsl(45,30%,92%)] truncate">{p.name}</div>
                      <div className="text-[10px] text-[hsl(45,15%,50%)] font-body capitalize">{p.faction}</div>
                    </div>
                  </div>
                  {owned ? (
                    <Badge className="text-[8px] bg-emerald-500/15 text-emerald-300 border-emerald-500/30">
                      Claimed{owned.isActive ? " · Active" : ""}
                    </Badge>
                  ) : (
                    <span className="text-[9px] text-[hsl(45,15%,45%)] font-body">Available</span>
                  )}
                </button>
              );
            })}
          </div>
          {filtered.length === 0 && (
            <div className="fantasy-panel p-8 text-center text-[hsl(45,15%,55%)] font-body">
              No characters match this filter.
            </div>
          )}
        </div>

        {selected && (
          <div className="lg:w-[320px] flex-shrink-0 space-y-4">
            <section className={`fantasy-panel p-5 ${factionColors[selected.faction]}`}>
              <div className="flex items-center gap-3 mb-3">
                <div
                  className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl"
                  style={{ background: selected.classColor + "30", border: `2px solid ${selected.classColor}50` }}
                >
                  {CLASSES.find((c) => c.id === selected.classId)?.icon}
                </div>
                <div>
                  <h3 className="font-heading text-lg text-[hsl(45,30%,92%)]" style={{ WebkitTextFillColor: "unset" }}>
                    {selected.name}
                  </h3>
                  <div className="flex gap-1.5">
                    <Badge
                      variant="outline"
                      className="text-[9px] uppercase"
                      style={{ borderColor: selected.classColor + "60", color: selected.classColor }}
                    >
                      {selected.classId}
                    </Badge>
                    <Badge
                      variant="outline"
                      className="text-[9px] uppercase border-[hsl(43,60%,30%)]/40 text-[hsl(43,85%,55%)]"
                    >
                      {selected.faction}
                    </Badge>
                  </div>
                </div>
              </div>
              <p className="text-xs text-[hsl(45,15%,60%)] font-body leading-relaxed">{selected.lore}</p>
            </section>

            <section className="fantasy-panel p-4">
              <div className="text-[10px] text-[hsl(45,15%,55%)] uppercase tracking-wider font-body mb-2">Base Stats</div>
              <div className="grid grid-cols-4 gap-2">
                {Object.entries(selected.baseStats).map(([k, v]) => (
                  <div key={k} className="text-center">
                    <div className="text-[10px] text-[hsl(45,15%,50%)] font-body">{k}</div>
                    <div className="text-sm font-heading gold-text">{v}</div>
                    <div className="w-full bg-black/30 h-1 rounded mt-0.5">
                      <div className="h-full rounded" style={{ width: `${(v / 7) * 100}%`, background: selected.classColor }} />
                    </div>
                  </div>
                ))}
              </div>
            </section>

            <section className="fantasy-panel p-4 space-y-2">
              {claimedByPrefab.has(selected.id) ? (
                <>
                  <Button
                    className="w-full gilded-button text-xs"
                    disabled={!!busy}
                    onClick={() => play(claimedByPrefab.get(selected.id)!, "warlords")}
                  >
                    <Play className="w-3.5 h-3.5 mr-1.5" /> Launch Warlords
                  </Button>
                  <Button
                    variant="outline"
                    className="w-full text-xs border-[hsl(43,60%,30%)]/40"
                    disabled={!!busy}
                    onClick={() => setActive(claimedByPrefab.get(selected.id)!)}
                  >
                    Set as active hero
                  </Button>
                </>
              ) : (
                <Button
                  className="w-full gilded-button text-xs"
                  disabled={!!busy}
                  onClick={() => claim(selected)}
                >
                  {busy === `claim-${selected.id}` ? (
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
                  ) : (
                    <>
                      <UserPlus className="w-3.5 h-3.5 mr-1.5" /> Claim for {player.username}
                    </>
                  )}
                </Button>
              )}
            </section>
          </div>
        )}
      </div>
    </div>
  );
}
