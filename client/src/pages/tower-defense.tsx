...
type FxKind = "fire" | "ice" | "wind" | "explosion";
...
const FX_DURATION: Record<FxKind, number> = { fire: 0.7, ice: 0.8, wind: 0.6, explosion: 0.9 };
...
function drawProcFx(ctx: CanvasRenderingContext2D, kind: FxKind, x: number, y: number, t: number, scale: number, seed: number, now: number) {
  const a = 1 - t;
  ctx.save();
  ctx.globalCompositeOperation = "lighter";

  if (kind === "fire") {
    const baseR = 28 * scale * (0.6 + (1 - t) * 0.6);
    const g1 = ctx.createRadialGradient(x, y, 0, x, y, baseR * 1.9);
    g1.addColorStop(0, `rgba(255,150,40,${a * 0.55})`);
    g1.addColorStop(0.4, `rgba(255,80,0,${a * 0.32})`);
    g1.addColorStop(1, "rgba(80,0,0,0)");
    ctx.fillStyle = g1;
    ctx.beginPath(); ctx.arc(x, y, baseR * 1.9, 0, Math.PI * 2); ctx.fill();
    for (let i = 0; i < 5; i++) {
      const ang = (i / 5) * Math.PI * 2 + now * 0.005 + seed;
      const wob = Math.sin(now * 0.02 + i + seed) * 0.3;
      const r = baseR * (0.55 + wob);
      const ox = Math.cos(ang) * baseR * 0.35;
      const oy = Math.sin(ang) * baseR * 0.35 - baseR * 0.25;
      const g2 = ctx.createRadialGradient(x + ox, y + oy, 0, x + ox, y + oy, r);
      g2.addColorStop(0, `rgba(255,255,210,${a * 0.9})`);
      g2.addColorStop(0.5, `rgba(255,150,30,${a * 0.7})`);
      g2.addColorStop(1, "rgba(200,30,0,0)");
      ctx.fillStyle = g2;
      ctx.beginPath(); ctx.arc(x + ox, y + oy, r, 0, Math.PI * 2); ctx.fill();
    }
    const g3 = ctx.createRadialGradient(x, y - baseR * 0.1, 0, x, y - baseR * 0.1, baseR * 0.5);
    g3.addColorStop(0, `rgba(255,255,255,${a * 0.9})`);
    g3.addColorStop(1, "rgba(255,255,255,0)");
    ctx.fillStyle = g3;
    ctx.beginPath(); ctx.arc(x, y - baseR * 0.1, baseR * 0.5, 0, Math.PI * 2); ctx.fill();
  } else if (kind === "ice") {
    const r = 32 * scale * (0.5 + t * 0.7);
    ctx.strokeStyle = `rgba(180,230,255,${a * 0.75})`;
    ctx.lineWidth = 2;
    ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.stroke();
    const g = ctx.createRadialGradient(x, y, 0, x, y, r * 0.85);
    g.addColorStop(0, `rgba(220,250,255,${a * 0.65})`);
    g.addColorStop(0.6, `rgba(0,200,255,${a * 0.3})`);
    g.addColorStop(1, "rgba(0,80,150,0)");
    ctx.fillStyle = g;
    ctx.beginPath(); ctx.arc(x, y, r * 0.85, 0, Math.PI * 2); ctx.fill();
    const shardLen = r * 1.25;
    for (let i = 0; i < 6; i++) {
      const ang = (i / 6) * Math.PI * 2 + seed + t * 0.4;
      const sx = x + Math.cos(ang) * r * 0.3;
      const sy = y + Math.sin(ang) * r * 0.3;
      const ex = x + Math.cos(ang) * shardLen;
      const ey = y + Math.sin(ang) * shardLen;
      const w = 6 * scale * (1 - t * 0.6);
      const px = -Math.sin(ang) * w * 0.5;
      const py = Math.cos(ang) * w * 0.5;
      ctx.fillStyle = `rgba(210,245,255,${a * 0.85})`;
      ctx.beginPath();
      ctx.moveTo(ex, ey);
    ...
    const r = 30 * scale;
    const spin = now * 0.015 + seed;
    for (let i = 0; i < 3; i++) {
      const ringR = r * (0.5 + i * 0.3);
      const off = i * 0.7;
      const sp = 1 + i * 0.3;
      ctx.strokeStyle = `rgba(220,250,255,${a * 0.55})`;
      ctx.lineWidth = 3 - i * 0.5;
      ctx.beginPath();
      ctx.arc(x, y, ringR, spin * sp + off, spin * sp + off + Math.PI * 1.4);
      ctx.stroke();
      ctx.strokeStyle = `rgba(180,240,255,${a * 0.25})`;
      ctx.lineWidth = (3 - i * 0.5) * 1.5;
      ctx.beginPath();
      ctx.arc(x, y, ringR, spin * sp + off - 0.8, spin * sp + off);
      ctx.stroke();
    }
    const gc = ctx.createRadialGradient(x, y, 0, x, y, r * 0.6);
    gc.addColorStop(0, `rgba(220,255,255,${a * 0.4})`);
    gc.addColorStop(1, "rgba(100,200,255,0)");
    ctx.fillStyle = gc;
    ctx.beginPath(); ctx.arc(x, y, r * 0.6, 0, Math.PI * 2); ctx.fill();
  } else if (kind === "explosion") {
    if (t < 0.2) {
      const fa = 1 - t / 0.2;
      const fr = 50 * scale * (1 + t * 2);
      const g = ctx.createRadialGradient(x, y, 0, x, y, fr);
      g.addColorStop(0, `rgba(255,255,255,${fa})`);
      g.addColorStop(0.4, `rgba(255,240,180,${fa * 0.8})`);
      g.addColorStop(1, "rgba(255,180,0,0)");
      ctx.fillStyle = g;
      ctx.beginPath(); ctx.arc(x, y, fr, 0, Math.PI * 2); ctx.fill();
    }
    const ballR = 40 * scale * (0.4 + t * 0.9);
    const g2 = ctx.createRadialGradient(x, y, 0, x, y, ballR);
    g2.addColorStop(0, `rgba(255,240,180,${a * 0.9})`);
    g2.addColorStop(0.4, `rgba(255,120,30,${a * 0.7})`);
    g2.addColorStop(0.8, `rgba(180,40,0,${a * 0.4})`);
    g2.addColorStop(1, "rgba(60,20,0,0)");
    ctx.fillStyle = g2;
    ctx.beginPath(); ctx.arc(x, y, ballR, 0, Math.PI * 2); ctx.fill();
    if (t > 0.1) {
      const rt = (t - 0.1) / 0.9;
      const rr = 60 * scale * rt;
      ctx.strokeStyle = `rgba(255,220,120,${(1 - rt) * 0.7})`;
    ...
    if (t > 0.3) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const dx = Math.cos(ang) * ballR * 0.6 * st;
        const dy = Math.sin(ang) * ballR * 0.6 * st - st * 20 * scale;
        const sR = 12 * scale * (1 + st);
        const sg = ctx.createRadialGradient(x + dx, y + dy, 0, x + dx, y + dy, sR);
        sg.addColorStop(0, `rgba(80,70,60,${(1 - st) * 0.4})`);
        sg.addColorStop(1, "rgba(40,30,20,0)");
        ctx.fillStyle = sg;
        ctx.beginPath(); ctx.arc(x + dx, y + dy, sR, 0, Math.PI * 2); ctx.fill();
      }
    }
  }
  ctx.restore();
...
export default function TowerDefense() {
  ...
  const addFx = useCallback((x: number, y: number, kind: FxKind, scale?: number) => {
    const dur = FX_DURATION[kind];
    effectsR.current.push({
      x, y, src: "", life: dur, maxLife: dur, scale: scale || 1,
      sheetType: "single", frameCount: 1, frameW: 0, frameH: 0, horizontal: false,
      kind, seed: Math.random() * Math.PI * 2,
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const loop = (ts: number) => {
        ...
        projsR.current.forEach(p => {
          ...
          if (dist(p.x, p.y, tgt.x, tgt.y) < 15) {
            ...
            if (p.type === 0 || p.type === 3) {
              ...
              spark(tgt.x, tgt.y, "#ff6600", 12);
              spark(tgt.x, tgt.y, "#ffcc44", 6);
              enemiesR.current.forEach(e2 => { if (e2.id !== tgt!.id && dist(e2.x, e2.y, tgt!.x, tgt!.y) < 60) e2.hp -= p.damage * 0.4; });
            } else if (p.type === 2) {
              addFx(tgt.x, tgt.y, "wind", hitScale);
              addEffect(tgt.x, tgt.y, "damage", hitScale * 0.6);
              spark(tgt.x, tgt.y, "#8855ff", 7);
              spark(tgt.x, tgt.y, "#c8e0ff", 5);
            } else if (p.type === 4) {
              addFx(tgt.x, tgt.y, "ice", hitScale);
              spark(tgt.x, tgt.y, "#00E5FF", 8);
              spark(tgt.x, tgt.y, "#ffffff", 4);
            } else {
            ...
            if (tgt.hp <= 0) {
              ...
              addFx(tgt.x, tgt.y, "explosion", tgt.isBoss ? 1.6 : 1.0);
      ...
      effectsR.current.forEach(ef => {
        ...
        if (ef.kind) {
          drawProcFx(ctx, ef.kind, ef.x, ef.y, elapsed, ef.scale, ef.seed || 0, performance.now());
          return;
        }
